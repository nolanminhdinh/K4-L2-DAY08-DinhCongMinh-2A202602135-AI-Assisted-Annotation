...
2. **Lỗi pre-label đã phát hiện và xử lý (`REVIEW_LOG.csv`):**
   - *Ca thêm mới (added):* Đúng như dự đoán độc lập, AI bỏ sót hoàn toàn chiếc xe tối đang tiến vào ở mép dưới (`frame_0182.jpg`, `cx=0.430, cy=0.945`). Thân xe nhìn rõ nắp capo và kính lái, đã được bổ sung box ôm sát thân xe theo quy tắc xe bị cắt ở mép ảnh. Thêm 1 xe khác ở mép dưới phải (`cx=0.825, cy=0.930`). Tại `frame_0099.jpg`, thêm 1 SUV tối màu ở làn giữa (`cx=0.430, cy=0.890`) và 1 xe ở làn đối diện.
   - *Ca xóa box trùng (deleted):* Tại `frame_0182.jpg`, AI sinh ra 2 box trùng nhau cho cùng một xe ở làn giữa xa (`cx=0.483, cy=0.472` và `cx=0.483, cy=0.460`); đã xóa 1 box thừa. Tại `frame_0331.jpg`, xuất hiện cụm 3 box chồng lấn trên cùng một xe (`cx=0.369`, `0.391`, `0.409`), đã xóa 2 box thừa.
   - *Ca chỉnh sửa (edited):* Tại `frame_0187.jpg` (`cx=0.608, cy=0.890`), box gợi ý ban đầu có chiều cao bất thường $h=0.182$ do ôm trọn vệt sáng đèn pha loang trên đường; đã thu gọn cạnh dưới lên sát cụm đèn trước và gầm xe ($h=0.105$). Tại `frame_0227.jpg`, chiếc xe tải thùng lớn (box truck) bị AI cắt làm 2 box rời rạc (1 box gầm/đèn $cx=0.279$ và 1 box nóc thùng $cx=0.298$); đã xóa box gầm và mở rộng box nóc ($cx=0.294, cy=0.545, w=0.115, h=0.185$) bao trọn toàn bộ xe tải từ gầm đến mui thùng xe.
   - *Ca chấp nhận (accepted):* Xe SUV màu trắng ở làn phải (`frame_0182.jpg`, `cx=0.785, cy=0.562`) và xe ô tô ở mép phải (`frame_0369.jpg`, `cx=0.852, cy=0.628`) có box ôm khít thân xe, viền chuẩn, không dính vệt đèn nên được giữ nguyên.
